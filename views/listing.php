    ...
    <div class="tg-joblisting tg-dashboardmanagejobs">
        <div class="tg-dashboardhead">
            <div class="tg-dashboardtitle">
                <h2><?php esc_html_e('Manage Articles', 'cannalisting_core'); ?><?php do_action('cannalisting_get_tooltip','section','articles');?></h2>
            </div>
            <div class="tg-btnaddservices">
                <a href="<?php cannalisting_Profile_Menu::cannalisting_profile_menu_link($profile_page, 'articles', $url_identity, '', 'add'); ?>"><?php esc_html_e('Add New Article', 'cannalisting_core'); ?></a>
            </div>
        </div>
        <?php if ($query->have_posts()) { ?>
            <div class="tg-sortfilters">
                <form class="form-sort-articles" method="get" action="<?php cannalisting_Profile_Menu::cannalisting_profile_menu_link($profile_page, 'articles', $url_identity, '', 'listing'); ?>">
                    <div class="tg-sortfilter tg-sortby">
                        <div class="tg-select">
                            <select name="sort" class="sort_by">
                                <option value="ID" <?php echo isset($_GET['sort']) && $_GET['sort'] == 'ID' ? 'selected' : ''; ?>><?php esc_html_e('Latest articles at top', 'cannalisting_core'); ?></option>
                                ...
                                <option value="name" <?php echo isset($_GET['sort']) && $_GET['sort'] == 'name' ? 'selected' : ''; ?>><?php esc_html_e('Order by article name', 'cannalisting_core'); ?></option>
                        ...
                        <tr>
                            <td>
                                <figure class="tg-companylogo">
                                    <a class="tg-btnedite" href="<?php cannalisting_Profile_Menu::cannalisting_profile_menu_link($profile_page, 'articles', $url_identity, '', 'edit', $post->ID); ?>"><i class="lnr lnr-pencil"></i></a>
                                ...
                                <div class="tg-contentbox"> 
                                    ...
                                    <span><?php esc_html_e('By', 'cannalisting_core'); ?>:&nbsp;<?php echo esc_attr($get_username); ?></span>
                                    <span><?php esc_html_e('Status', 'cannalisting_core'); ?>:&nbsp;<?php echo apply_filters('cannalisting_print_post_status',$status,$post->ID); ?></span> 
                                </div>
                            </td>
                        </tr>
                        <?php
        ...
        <?php } else { ?>
            <div class="tg-dashboardappointmentbox">
                <?php cannalisting_Prepare_Notification::cannalisting_info(esc_html__('Information', 'cannalisting_core'), esc_html__('No articles found.', 'cannalisting_core')); ?>
            </div>
        <?php } ?>
    </div>
</div>
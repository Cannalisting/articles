 ...
 * @type insert
 */
if (!function_exists('fw_ext_cannalisting_process_articles')) {

    function fw_ext_cannalisting_process_articles() {
        ...
        $do_check = check_ajax_referer('cannalisting_article_nounce', 'cannalisting_article_nounce', false);
        if ($do_check == false) {
            $json['type'] = 'error';
        ...
        $title = !empty($_POST['article_title']) ? esc_attr($_POST['article_title']) : esc_html__('unnamed', 'cannalisting_core');
        $article_detail = do_shortcode($_POST['article_detail']);

        $attachment_id = !empty($_POST['attachment_id']) ? intval($_POST['attachment_id']) : '';
        $article_tags = !empty($_POST['article_tags']) ? cannalisting_sanitize_array($_POST['article_tags']) : array();
        ...
        if (isset($type) && $type === 'add') {
            ...
            if (isset($approve_articles) && $approve_articles === 'need_approval') {
                $status = 'pending';
                $json['message'] = esc_html__('Your article has submitted and will be publish after the review.', 'cannalisting_core');
            } else {
                $status = 'publish';
                $json['message'] = esc_html__('Article added successfully.', 'cannalisting_core');
            }
            ...
            $return_url = cannalisting_Profile_Menu::cannalisting_profile_menu_link($profile_page, 'articles', $current_user->ID, 'true', 'listing');
            ...
            if (intval($current_user->ID) === intval($post_author)) {
                ...
                $json['message'] = esc_html__('Article updated successfully.', 'cannalisting_core');
            } else {
                $json['type'] = 'error';
    ...
    add_action('wp_ajax_fw_ext_cannalisting_process_articles', 'fw_ext_cannalisting_process_articles');
    add_action('wp_ajax_nopriv_fw_ext_cannalisting_process_articles', 'fw_ext_cannalisting_process_articles');
}
 ...
 * @type delete
 */
if (!function_exists('fw_ext_cannalisting_delete_articles')) {

    function fw_ext_cannalisting_delete_articles() {
        global $current_user, $wp_roles, $userdata;
        ...
        if (!empty($post_id) && intval($current_user->ID) === intval($post_author)) {
            ...
            $json['type'] = 'success';
            $json['message'] = esc_html__('Article deleted successfully.', 'cannalisting_core');
            echo json_encode($json);
            die;
        } else {
            $json['type'] = 'error';
    ...
    add_action('wp_ajax_fw_ext_cannalisting_delete_articles', 'fw_ext_cannalisting_delete_articles');
    add_action('wp_ajax_nopriv_fw_ext_cannalisting_delete_articles', 'fw_ext_cannalisting_delete_articles');
}
